...
- **三端权限模型**：基于 Spring Security + JWT 的权限控制，区分管理员、医生、居民角色
...
系统包含 26 张核心数据表，采用逻辑删除与乐观锁机制保障数据完整性。
...
│   │   └── user/                   # 居民端页面
│   ├── components/                 # 公共组件
│   ├── common/                     # 公共 API 和 Mixins
│   ├── static/                     # 静态资源
│   ├── store/                      # Vuex 状态管理
│   └── utils/                      # 工具函数
│
└── vaccine_system/                 # 后端 Spring Boot 项目
    ├── src/main/java/com/tjut/edu/
    │   ├── controller/             # 控制层
    │   │   ├── admin/              # 管理员接口
    │   │   ├── doctor/             # 医生接口
    │   │   ├── user/               # 用户接口
...
- **JWT 无状态认证**：基于 Token 的身份验证，支持 token 刷新机制
